...
import { COLOR_MODES, DEFAULT_COLOR_MODE, normalizeColorMode } from './profileAppearance'
...
// keep meaning what it has always rendered, so the default is 'dark'.
//
// AND WHY NOTHING HERE READS THE VIEWER'S PREFERENCE
// The mode is a property of the document, not of the person looking at it: a
// profile is sent to somebody, and the sender has to know what they will see.
// There was a third choice - 'system' - that resolved against
// prefers-color-scheme, which meant the same link rendered two different
// pages to two readers. It is gone from the control, and a profile that
// still stores it normalises to the default above rather than following
// anybody's laptop.
//
// THE FLASH
...
// Re-exported so every component keeps importing them from here, which is
// where they have always been imported from. They are declared in
// profileAppearance.js because the route handler needs them too and cannot
// reach into this file - see the note at the top of that one.
export { COLOR_MODES, DEFAULT_COLOR_MODE, normalizeColorMode }

const ATTR = 'data-hp-profile-mode'
const CACHE_PREFIX = 'hp_profile_mode:'

// Whatever is stored -> 'light' | 'dark'. Kept as its own export because
// every caller reads it by this name, and because "resolve" is still the
// right word: what is stored can be null, or a value this build no longer
// offers, and both have to land on something paintable.
