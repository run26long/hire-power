...
// The same list, narrowed to what the Portfolio shows. Derived from the one
// above rather than written out again, so a type added there is offered here
// the moment it is visual and never needs a second edit to be consistent.
export const VISUAL_ACCEPT_ATTRIBUTE = Object.entries(UPLOAD_TYPES)
  .filter(([, type]) => type.media_class === 'image' || type.media_class === 'video')
  .map(([mime]) => mime)
  .join(',')
