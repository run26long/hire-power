...
// The stem every named core is built on. Declared once because two things need
// it: the label is assembled from it, and the hub's tile row takes it back off
// again to show the direction on its own.
export const CORE_LABEL_PREFIX = 'Core Resume - '
...
/**
 * The direction out of a label this module wrote, or null when it carries none.
 *
 * For the places that have already been handed the finished label and want the
 * short form: the hub's tile row is three tiles wide, so "Core Resume - Athletic
 * Coaching" clips there while the direction alone fits. The card above it, which
 * has the width, keeps the full name.
 *
 * Deliberately narrow. It only unwraps a string this module produced, and
 * returns null for anything else rather than guessing at a name somebody typed.
 */
export function directionFromLabel(label) {
  const text = String(label || '').trim()
  if (!text.startsWith(CORE_LABEL_PREFIX)) return null
  return text.slice(CORE_LABEL_PREFIX.length).trim() || null
}
...
export function coreResumeLabel({ resume, lenses = [], currentLensName = null, coreCount = null } = {}) {
  ...
  return direction ? `${CORE_LABEL_PREFIX}${direction}` : 'Core Resume'
}
