    ...
    <Document hyphenationCallback={(w) => [w]}>
      <Page size="LETTER" style={{ fontFamily: f, fontSize: base, lineHeight: 1.2, color: '#1a1a1a', paddingTop: 36, paddingBottom: 36, paddingLeft: 52, paddingRight: 52, backgroundColor: '#ffffff' }}>
        ...
        <View fixed style={{ position: 'absolute', top: 14, left: 52, right: 52 }} render={({ pageNumber }) => pageNumber > 1 ? (
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingBottom: 4, borderBottomWidth: 0.5, borderBottomColor: '#cccccc' }}>
            <Text style={{ fontFamily: f, fontSize: base - 2, fontWeight: 'bold', textTransform: 'uppercase', color: '#888888' }}>{resumeData.fullName || ''}</Text>
        ...
        {(resumeData.sectionOrder || ['experience','education','skills','projects','certifications','volunteer','languages','additionalInfo']).map((section) => {
          switch(section) {
            ...
            case 'experience': {
              ...
              const renderGroup = (group, includeHeader = false) => {
                if (group.roles.length === 1) {
                  ...
                  return (
                    <>
                      <View wrap={false}>
                        {includeHeader && <SH title={resumeData.sectionTitles?.experience || 'Professional Experience'} />}
              ...
              return (
                <View key="experience">
                  <View style={{ paddingLeft: Math.round(10*sp), paddingRight: Math.round(10*sp), marginBottom: restGroups.length > 0 ? Math.round(8*sp) : 0 }}>
                    {renderGroup(firstGroup, true)}
                  </View>
                  {restGroups.map((group, gi) => (
                    <View key={gi+1} style={{ paddingLeft: Math.round(10*sp), paddingRight: Math.round(10*sp), marginBottom: gi < restGroups.length - 1 ? Math.round(8*sp) : 0 }}>
                      {renderGroup(group)}
                    </View>
                  ))}
                </View>
            ...
            case 'education': {
              ...
              return (
                <View key="education">
                  ...
                  {restGroups.map((group, gi) => (
                    <View key={gi+1} wrap={false} style={{ paddingLeft: Math.round(10*sp), paddingRight: Math.round(10*sp), marginBottom: gi < restGroups.length - 1 ? Math.round(8*sp) : 0 }}>
