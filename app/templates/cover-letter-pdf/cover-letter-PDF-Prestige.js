...
export default function CoverLetterPDFPrestige({ coverLetterData, font = 'EB Garamond', fontSize = 11, spacing = 1, accentColor = '#5b4fcf' }) {
    ...
    <Document hyphenationCallback={(w) => [w]}>
      <Page size="LETTER" style={{ fontFamily: f, fontSize: base, lineHeight: 1.2, color: '#1a1a1a', backgroundColor: '#ffffff', paddingTop: Math.round(20*sp), paddingBottom: Math.round(36*sp) }}>
        ...
        <View style={{ marginTop: -Math.round(20*sp), paddingTop: Math.round(30*sp), paddingLeft: Math.round(52*sp), paddingRight: Math.round(52*sp) }}>
          <Text style={{ fontFamily: f, fontSize: 22, fontWeight: 'bold', textTransform: 'uppercase', color: '#1a1a1a', marginBottom: Math.round(16*sp) }}>{cl.candidateName || ''}</Text>
        </View>

        {/* Contact band */}
        <View style={{ backgroundColor: hexToRgba(accent, 0.094), borderTopWidth: 2, borderTopColor: accent, paddingTop: Math.round(6*sp), paddingBottom: Math.round(6*sp), paddingLeft: Math.round(52*sp), paddingRight: Math.round(52*sp), marginBottom: Math.round(8*sp) }}>
