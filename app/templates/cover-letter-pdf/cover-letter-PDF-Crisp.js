...
export default function CoverLetterPDFCrisp({ coverLetterData, font = 'Source Serif 4', fontSize = 11, spacing = 1 }) {
  const cl = coverLetterData || {}
  const base = fontSize
  const sp = spacing
  const f = font || 'Source Serif 4'
  ...
  return (
    <Document hyphenationCallback={(w) => [w]}>
      <Page size="LETTER" style={{ fontFamily: f, fontSize: base, lineHeight: 1.2, color: '#1a1a1a', paddingTop: 36, paddingBottom: 36, paddingLeft: 43, paddingRight: 43, backgroundColor: '#ffffff' }}>
        ...
        {/* HEADER */}
        <Text style={{ fontFamily: f, fontSize: 20, fontWeight: 'bold', textTransform: 'uppercase', textAlign: 'center', marginBottom: Math.round(13*sp) }}>{cl.candidateName || ''}</Text>
