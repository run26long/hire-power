...
For each missing keyword, determine whether any item in the confirmed knowledge meaningfully covers it. Judge by meaning and concept, not by matching words. The knowledge items are written in plain everyday language while job keywords often use technical terms, acronyms, or industry jargon. A keyword is covered when the candidate's described experience is the same concept the keyword refers to, regardless of vocabulary. For example: a knowledge item describing "storing facts in a database and retrieving relevant ones to include in prompts" covers the keyword "RAG" or "Retrieval-Augmented Generation." An item describing "choosing which model handles which task based on cost and quality" covers "model routing" or "performance budgets." Translate the jargon to its underlying concept, then check whether the candidate's experience covers that concept.

If the candidate built, implemented, designed, or documented something that does what the keyword describes, the keyword is covered. Having created the capability is stronger evidence than having used someone else's version of it. "Built a scoring system with explicit rubrics" covers "LLM evaluation pipelines." "Wrote written specs, architecture documentation, and release rules" covers "deployment playbooks." A keyword is covered if the candidate's confirmed experience would satisfy that requirement to a hiring manager reading their resume.
