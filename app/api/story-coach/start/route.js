...
// ============================================================================
// STORY COACHING — DIALOGUE SYSTEM PROMPT
// Source of truth: STORY_COACHING_PROMPT.md (Part 1)
...
// We send a compact summary of relevant experience, not the entire resume,
// to keep dialogue context light.
// ============================================================================

function extractRelevantExperience(resumeData) {
...
// ============================================================================
// POST /api/story-coach/start
// Opens a new coaching session for one Power Analysis item.
// Creates the interview_stories row, fires opening Sonnet call, saves dialogue.
//
// Request body: {
//   jobCardId: string,
//   itemType: 'core_power' | 'hidden_power' | 'power_gap',
//   itemIndex: number,
// }
// ============================================================================
...
export async function POST(request) {
  try {
    ...
    // ---- INPUT ----
    const { jobCardId, itemType, itemIndex } = await request.json();
    if (!jobCardId || !itemType || itemIndex === undefined || itemIndex === null) {
      return Response.json({ error: 'jobCardId, itemType, and itemIndex required' }, { status: 400 });
    }
    ...
    const { data: powerAnalysis, error: paError } = await supabase
      .from('power_analysis')
      .select('id, core_power, hidden_power, power_gaps')
      .eq('job_card_id', jobCardId)
      .eq('user_id', userId)
      .single();
    if (paError || !powerAnalysis) {
      return Response.json({ error: 'POWER_ANALYSIS_NOT_FOUND' }, { status: 404 });
    }

    // ---- RESOLVE THE PA ITEM ----
    let items;
    if (itemType === 'core_power') items = powerAnalysis.core_power;
    else if (itemType === 'hidden_power') items = powerAnalysis.hidden_power;
    else items = powerAnalysis.power_gaps;

    if (!Array.isArray(items) || itemIndex >= items.length) {
      return Response.json({ error: 'ITEM_NOT_FOUND' }, { status: 404 });
    }
    const item = items[itemIndex];

    // Derive skill name and context text based on item type
    const itemSkill = item.skill || item.gap || 'Unknown skill';
    let itemContext;
    if (itemType === 'core_power') itemContext = item.evidence || '';
    else if (itemType === 'hidden_power') itemContext = item.evidence_reframe || '';
    else itemContext = item.bridge_strategy || '';
    ...
    if (jobCard.resume_id) {
      ...
      if (resume?.resume_data) {
        resumeExperience = extractRelevantExperience(resume.resume_data);
      }
    }

    // ---- CHECK FOR EXISTING SESSION (resume if it exists, not yet finalized) ----
    ...
    if (existing) {
      if (existing.coaching_complete) {
        // Already finalized — don't re-open. UI should handle this by showing the story.
        return Response.json({ error: 'STORY_ALREADY_COMPLETE', storyId: existing.id }, { status: 400 });
      }
      // Resume the in-progress session
      return Response.json({
        storyId: existing.id,
        itemType: existing.item_type,
        itemIndex: existing.item_index,
        itemSkill: existing.item_skill,
        itemContext,
        dialogue: existing.coaching_dialogue || [],
        resumed: true
      });
    }

    // ---- BUILD OPENING USER MESSAGE ----
    const openingUserMessage = `ITEM TYPE: ${itemType}
ITEM SKILL: ${itemSkill}
ITEM CONTEXT: ${itemContext}

JOB TITLE: ${jobCard.title}
JOB COMPANY: ${jobCard.company || 'Unknown'}

RESUME EXPERIENCE:
${resumeExperience || '(No resume experience available)'}

Let's start the coaching session.`;

    // ---- FIRE OPENING SONNET CALL ----
    ...
    const openingAssistantMessage = response.content[0].text;

    // ---- BUILD INITIAL DIALOGUE ----
    // We store both turns so the next /message call has full context.
    const dialogue = [
      { role: 'user', content: openingUserMessage },
      { role: 'assistant', content: openingAssistantMessage }
    ];
    ...
    const { data: newStory, error: insertError } = await supabase
      ...
      .insert({
        ...
        power_analysis_id: powerAnalysis.id,
        item_type: itemType,
        item_index: itemIndex,
        item_skill: itemSkill,
        coaching_dialogue: dialogue,
    ...
    return apiError(error, "We couldn't start coaching for this item. Try again in a moment.");
  }
}