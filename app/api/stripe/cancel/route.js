...
export async function POST(req) {
  try {
    ...
    if (updateError) {
      // Stripe cancellation succeeded but our DB didn't update.
      // Log loudly so we can reconcile manually if it ever happens.
      console.error('CRITICAL: Stripe cancel succeeded but profile update failed', {
        userId,
      ...
      return apiError(
        updateError,
        "Your cancellation didn't save correctly. Please email hired@hirepowerai.com so we can fix this for you.",
        500
      );
    }
    ...
    return apiError(error, "We couldn't process your cancellation. Please try again, or email hired@hirepowerai.com if it keeps failing.");
  }
}